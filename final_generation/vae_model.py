...
class UNetCVAE(nn.Module):
    def __init__(self, latent_dim: int = 128, in_ch: int = 3, num_classes: int = 50):
        super().__init__()
        ...
        self.fc_z = nn.Linear(latent_dim + num_classes, self.flatten_dim)
    ...
    def encode(self, x, y):
        ...
        mu, logvar = self.fc_mu(h_cat), self.fc_logvar(h_cat)
        return mu, logvar, (s1, s2, s3), y_onehot
    ...
    def decode(self, z, skips, y_onehot):
        s1, s2, s3 = skips
        z_cat = torch.cat([z, y_onehot], dim=1)
        h = self.fc_z(z_cat).view(-1, 256, 8, 8)
        d1 = self.up1(h, s3)
        d2 = self.up2(d1, s2)
        d3 = self.up3(d2, s1)
    ...
    def forward(self, x, y):
        mu, logvar, skips, y_onehot = self.encode(x, y)
        z = self.reparameterize(mu, logvar)
        recon = self.decode(z, skips, y_onehot)
